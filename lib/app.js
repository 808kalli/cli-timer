...
const { renderFrame, formatTime, SLIME_WIDTH, SLIME_MARGIN } = require('./render');
const {
  HIDE_CURSOR,
  SHOW_CURSOR,
  ENTER_ALT_SCREEN,
...
function run(argv) {
  ...
  let paused = false;
  let tick = 0;
  let slimeX = -1; // -1 until the first step centers it
  const laps = [];

  // The slime wanders instead of pacing: each step is left, right or a beat
  // standing still. Position accumulates here rather than in the renderer so
  // renderFrame stays a pure function of the state handed to it.
  function stepSlime() {
    const interior = (process.stdout.columns || 80) - 2;
    const maxX = Math.max(SLIME_MARGIN, interior - SLIME_WIDTH - SLIME_MARGIN);
    if (slimeX < 0) {
      slimeX = Math.floor((SLIME_MARGIN + maxX) / 2);
      return;
    }
    const roll = Math.random();
    const step = roll < 0.25 ? 0 : roll < 0.625 ? -1 : 1;
    slimeX = Math.min(maxX, Math.max(SLIME_MARGIN, slimeX + step));
  }
  ...
  if (args.slime) stepSlime(); // center it before the first paint
  draw();
