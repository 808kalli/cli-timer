...
// Startle-jump on hover: a beat looking at you, then up, then back down.
// Timed rather than tied to the animation tick, which is far too coarse.
const STARTLE_DELAY_MS = 200;
const STARTLE_AIR_MS = 500;
const {
  HIDE_CURSOR,
  SHOW_CURSOR,
  ENTER_ALT_SCREEN,
...
function run(argv) {
  ...
  function startStartle() {
    startleTimers.push(
      ...
      setTimeout(() => {
        ...
        draw();
      }, STARTLE_DELAY_MS + STARTLE_AIR_MS),
    );
  }
  ...
  function handleMouse(data) {
    ...
    if (over !== slime.hovered) {
      ...
      clearStartle();
      slime.airborne = false;
      updateSlime(slime, (process.stdout.columns || 80) - 2, paused);
      draw();
