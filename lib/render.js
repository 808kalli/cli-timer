...
// A slime built from the same tiles as the digits. The eyes are unlit tiles in
// the middle row — enclosed on every side, so they read as holes in a face
// rather than notches in the silhouette — and closing them just fills those
// two tiles back in. The body never changes shape, so nothing flickers.
const SLIME_OPEN = ['██████', '█ ██ █', '██████'];
const SLIME_SHUT = ['██████', '██████', '██████'];
const SLIME_WIDTH = SLIME_OPEN[0].length;
const SLIME_ROWS = SLIME_OPEN.length;
const SLIME_MARGIN = 1;
const BLINK_EVERY = 24;

function buildSlimeRows(tick, paused, interior, color, x) {
  const maxX = Math.max(SLIME_MARGIN, interior - SLIME_WIDTH - SLIME_MARGIN);
  const left = Math.min(maxX, Math.max(SLIME_MARGIN, x));
  const asleep = paused || tick % BLINK_EVERY === BLINK_EVERY - 1;

  return (asleep ? SLIME_SHUT : SLIME_OPEN).map((row) => {
...
function renderFrame({
  ...
  const slimeRows = showSlime ? buildSlimeRows(tick, paused, interior, color, slimeX) : [];
...
module.exports = { renderFrame, formatTime, SLIME_WIDTH, SLIME_MARGIN };
