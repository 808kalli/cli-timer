...
const { GLYPH_HEIGHT, BAR_UNITS, glyphRows, glyphWidth } = require('./font');
...
// the terminal could hold) rather than stretching to fill the screen. Only
// the horizontal bar length scales with hRepeat — the vertical strokes stay
// a single column — so digitWidth(hRepeat) = 2 + BAR_UNITS * hRepeat.
function pickScale(chars, cols, rows) {
  ...
  let fixedWidth = numGaps * gap;
  let scalingUnits = 0;
  for (const c of chars) {
    if (c === ':' || c === ' ') {
      fixedWidth += glyphWidth(c, 1);
    } else {
      fixedWidth += 2;
      scalingUnits += BAR_UNITS;
    }
  }

  const hRepeatMax =
    scalingUnits > 0 ? Math.floor((availableWidth - fixedWidth) / scalingUnits) : MAX_HREPEAT;
  const hRepeat = Math.max(1, Math.min(hRepeatMax, MAX_HREPEAT));
