...
function buildRecordingStatus(elapsedSeconds) {
  const dot = elapsedSeconds % 2 === 0 ? '●' : '○';
  return fg(...NEON_RED, `${dot} REC`);
}
...
function renderFrame({
  ...
  let statusRow;
  if (finished) {
    statusRow = fg(...NEON_RED, '*** TIME\'S UP ***');
  } else if (isStopwatch) {
    statusRow = buildRecordingStatus(elapsedSeconds);
  } else {
    const fraction = totalSeconds > 0 ? Math.min(1, Math.max(0, 1 - remainingSeconds / totalSeconds)) : 1;
    statusRow = buildProgressBar(width - 10, fraction);
  }

  const contentBlock = [...digitRows, '', statusRow];
  ...
  out.push(bottomBorder(width, hint));
