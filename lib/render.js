...
function pickScale(chars, cols, rows) {
  ...
  // Height is additionally capped by hRepeat. A terminal that is tall but too
  // narrow to widen the digits would otherwise still stretch them vertically,
  // which is the one case where they came out elongated.
  const vRepeat = Math.max(
    1,
    Math.min(Math.floor(availableHeight / GLYPH_HEIGHT), MAX_VREPEAT, hRepeat),
  );
...
function buildSlimeRows(state, interior, color) {
  ...
  // The band is one row taller than the body, giving the jump room to stretch
  // upward and to clear the floor. Each row holds segments positioned relative
  // to the slime's left edge — the body at 0, a shout off to its left — so a
  // row can carry both.
  return slimeBand(state).map((segments) => {
    const cells = new Array(interior).fill(' ');
    for (const { text, dx } of segments) {
      let col = left + dx;
      // A shout with no room to its left flips to the slime's other side
      // rather than being clamped on top of the body.
      if (dx < 0 && col < 0) col = left + SLIME_WIDTH + 1;
      col = Math.max(0, Math.min(interior - text.length, col));
      for (let i = 0; i < text.length; i++) cells[col + i] = text[i];
    }
    return fg(...color, cells.join(''));
  });
}
...
function renderFrame({
  ...
  const seconds = isClock ? clockSeconds : isStopwatch ? elapsedSeconds : remainingSeconds;
  ...
  if (width - 2 < minContentWidth(chars) + 2) {
    ...
    digitRows = buildDigitRows(chars, pickScale(chars, width, height), color);
    digitsWide = true;
  }

  // The clock carries the date immediately left of the digits, stacked over
  // two lines to keep it narrow. It takes only the width left spare beside the
  // digits at their natural size — reserving space up front would cost a whole
  // scale step, halving the digits on an 80-column terminal. Where even the
  // short form will not fit, the date is dropped rather than the clock shrunk.
  if (isClock && dateLabel && digitsWide) {
    const [weekday, rest] = dateLabel.split(' | ');
    const noYear = rest.replace(/ \d{4}$/, ''); // Jul 27
    // Progressively shorter forms, each dropping the least useful part first:
    // the year, then the full weekday name.
    // Kept on one line for as long as it fits; wrapped onto two before any of
    // the wording is given up, and only then shortened.
    const candidates = [
      [`${weekday} | ${rest}`], // Monday | Jul 27 2026
      [weekday, rest], // Monday / Jul 27 2026
      [weekday, noYear], // Monday / Jul 27
      [weekday.slice(0, 3), noYear], // Mon / Jul 27
    ];
    const spare = interior - visibleLength(digitRows[0]);

    for (const lines of candidates) {
      const w = Math.max(...lines.map((l) => l.length));
      if (spare < w + 1) continue;
      const gap = spare >= w + 2 ? 2 : 1;
      const top = Math.max(0, Math.floor((digitRows.length - lines.length) / 2));
      digitRows = digitRows.map((row, i) => {
        const line = lines[i - top] || '';
        return fg(...theme.gray, line.padEnd(w)) + ' '.repeat(gap) + row;
      });
      break;
    }
  }
  ...
  const bottomLines = buildLapLines(
    laps,
    theme,
  ...
  const centeredHeight = interiorHeight - bottomLines.length - slimeRows.length;
  ...
  const hint = finished
    ...
    : isClock
      ? // Nothing to lap, pause or reset on a wall clock.
        'TAB invert · Q quit'
  ...
  for (const line of bottomLines) out.push(leftRow(width, line, color));
