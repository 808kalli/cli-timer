...
// A slime built from the same tiles as the digits. '#' is a lit tile; the eyes
// are left blank so the row background shows straight through them.
const SLIME_PIXEL_W = 2;
const SLIME_ROWS = 3;
const SLIME_SPRITES = {
  // Stretched mid-hop, then squashed on landing. Alternating the two while it
  // travels sideways reads as a hop without needing a taller band.
  stretch: ['######', '# ## #', '######'],
  squash: ['      ', '# ## #', '######'],
  // Eyes closed: asleep while paused, and for a single tick when blinking.
  shut: ['      ', '######', '######'],
};
const SLIME_PIXELS = SLIME_SPRITES.stretch[0].length;
const SLIME_WIDTH = SLIME_PIXELS * SLIME_PIXEL_W;
const BLINK_EVERY = 24;

const SLIME_MARGIN = 1;

function slimeSprite(tick, paused) {
  if (paused) return SLIME_SPRITES.shut;
  // Offset so tick 0 is not a blink — the first frame should have eyes.
  if (tick % BLINK_EVERY === BLINK_EVERY - 1) return SLIME_SPRITES.shut;
  return tick % 2 === 0 ? SLIME_SPRITES.stretch : SLIME_SPRITES.squash;
}

// Bounces between the two walls. Derived purely from the tick so the frame
// stays a function of state.
function slimeX(tick, span) {
  if (span <= 0) return 0;
  const p = tick % (span * 2);
  return p < span ? p : span * 2 - p;
}

function buildSlimeRows(tick, paused, interior, color) {
  const span = interior - SLIME_WIDTH - SLIME_MARGIN * 2;
  const x = SLIME_MARGIN + slimeX(tick, span);
  const sprite = slimeSprite(tick, paused);
  return sprite.map((row) => {
    let body = '';
    for (const px of row) body += (px === '#' ? '█' : ' ').repeat(SLIME_PIXEL_W);
    const line = ' '.repeat(x) + body;
...
function renderFrame({
  ...
  const slimeRows = showSlime ? buildSlimeRows(tick, paused, interior, color) : [];
