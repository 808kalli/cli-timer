...
function buildSlimeRows(state, interior, color) {
  ...
  // The band is one row taller than the body: normally the slime sits at the
  // bottom of it, and a startle-jump lifts it into the spare row on top.
  const body = slimeSprite(state);
  const band = state.airborne ? [...body, ''] : ['', ...body];

  return band.map((row) => {
    const line = row ? ' '.repeat(left) + row : '';
