...
const JUMP_POSES = {
  ...
  air: { height: 4, eyeRow: 2, bottom: 1 },
...
function slimeSprite(state) {
  ...
  if (pose) {
    ...
    return body(pose.height, pose.eyeRow, state.shut ? null : EYE_COLUMNS.center);
  }
...
function slimeBand(state) {
  ...
  const band = new Array(BAND_HEIGHT).fill('');
  const start = BAND_HEIGHT - bottom - sprite.length;
  for (let i = 0; i < sprite.length; i++) band[start + i] = sprite[i];
  return band;
}
