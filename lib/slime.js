...
function updateSlime(state, interior, paused) {
  ...
  if (state.hovered) {
    // Noticed the pointer: stops where it is and stares straight ahead,
    // eyes wide, until the mouse moves off it again.
    state.shut = false;
    state.look = 'center';
    state.blinkTicks = 0;
    return state;
...
const ROW_UP = 1;
const ROW_DOWN = 2;
// Startled by the pointer: each eye widens to two tiles.
