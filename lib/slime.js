...
function createSlime() {
  return {
    ...
    pose: null, // 'crouch' | 'stretch' | 'air' | 'land' during a startle-jump
    settling: false, // was hovered, needs to reset its trip once released
...
function updateSlime(state, interior, paused) {
  ...
  // Mid-jump it stays put rather than drifting sideways, even if the pointer
  // has already moved off.
  if (state.pose) return state;

  if (state.settling) {
    // Just released: abandon whatever trip was interrupted and stay put for a
    // while, so it holds its ground instead of darting off the moment the
    // pointer leaves.
...
// Builds a body `height` rows tall with the eyes on `eyeRow`. Any height of 3
// or more keeps a solid tile above and below the eyes.
function body(height, eyeRow, columns) {
  const rows = [];
  for (let r = 0; r < height; r++) rows.push('█'.repeat(WIDTH).split(''));
  if (columns) for (const c of columns) rows[eyeRow][c] = ' ';
  return rows.map((r) => r.join(''));
}

// The jump squashes and stretches rather than sliding a rigid block upwards:
// it crouches, launches tall and thin, sails at its normal height, then
// squashes again on landing. `bottom` is how far the body sits above the
// floor of the band.
//   idle    4 rows on the floor
//   crouch  3 rows, compressed on the floor
//   stretch 5 rows, drawn out on take-off
//   air     4 rows, one row clear of the floor
//   land    3 rows, compressed again on impact
const JUMP_POSES = {
  crouch: { height: 3, eyeRow: 1, bottom: 0 },
  stretch: { height: 5, eyeRow: 2, bottom: 0 },
  air: { height: 4, eyeRow: 2, bottom: 1, wide: true, speech: 'HI!' },
  land: { height: 3, eyeRow: 1, bottom: 0 },
};

// Eyes go wide for the airborne frame only — each one two tiles across,
// still with a solid tile all the way around it.
const EYE_COLUMNS_WIDE = [1, 2, 5, 6];

function slimeSprite(state) {
  const pose = JUMP_POSES[state.pose];
  if (pose) {
    // Mid-jump it always faces you, eyes open.
    const columns = pose.wide ? EYE_COLUMNS_WIDE : EYE_COLUMNS.center;
    return body(pose.height, pose.eyeRow, state.shut ? null : columns);
  }

  if (state.shut) return body(HEIGHT, ROW_DOWN, null);
  ...
  return body(HEIGHT, row, columns);
}

// Gap between the shout and the slime's left edge.
const SPEECH_GAP = 1;

// The body placed into the full band, bottom-aligned to the floor unless the
// pose lifts it clear. Each row is a list of segments positioned relative to
// the slime's left edge, so a shout can sit outside the body's own columns.
function slimeBand(state) {
  const sprite = slimeSprite(state);
  const pose = JUMP_POSES[state.pose];
  const bottom = pose ? pose.bottom : 0;
  const band = [];
  for (let i = 0; i < BAND_HEIGHT; i++) band.push([]);

  const start = BAND_HEIGHT - bottom - sprite.length;
  for (let i = 0; i < sprite.length; i++) band[start + i].push({ text: sprite[i], dx: 0 });

  // Thrown up and to the left, level with the top of the body.
  if (pose && pose.speech) {
    band[start].push({ text: pose.speech, dx: -(pose.speech.length + SPEECH_GAP) });
  }
  return band;
}
